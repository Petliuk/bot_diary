...
public class NotificationTimePickerHandler {
    ...
    public void handleCalendarDaySelection(Update update) throws TelegramApiException {
        ...
        try {
            // Перевірка довжини рядка перед отриманням підстроки
            if (callbackData.startsWith("CAL_DAYS_") && callbackData.length() > "CAL_DAYS_".length()) {
                int dayOfMonth = Integer.parseInt(callbackData.substring("CAL_DAYS_".length()));
                handleDateSelection(update, dayOfMonth);
            } else {
                System.err.println("Invalid callback data length: " + callbackData);
            }
        ...
        public SendMessage generateCalendarMessage(long chatId, YearMonth currentMonth) {
            ...
            keyboardRows.addAll(createDaysButtons(currentMonth, today, taskDeadline, chatId));
        ...
        private List<List<InlineKeyboardButton>> createDaysButtons(YearMonth currentMonth, LocalDate today, LocalDate taskDeadline, long chatId) {
